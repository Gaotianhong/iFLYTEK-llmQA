...
def main():
    ...
    args.log_path = os.path.join(args.logs_specs, args.name, "out_{}.log".format(time_suffix))

    args.checkpoint_path = os.path.join(args.logs_specs, args.name, "checkpoints")
    if is_master(args):
    ...
    if is_master(args):
        logging.info("Params:")
        params_file = os.path.join(args.logs_specs, args.name, "params_{}.txt".format(time_suffix))
    ...
    args.should_save = (args.logs_specs is not None and args.logs_specs != '' and args.logs_specs.lower() != 'none') and is_master(args)
